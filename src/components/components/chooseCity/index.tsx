...
const Content = (props: Props) => {
  ...
  return (
    <div>
      ...
      <div className='cc-choose-city-city'>
        {Object.keys(cities).map(item => {
          return (
            <div
              onClick={() => clickChat(item)}
              className='cc-choose-city-chat'
              key={item}>
              {item}
            </div>
          )
          ...
          Object.keys(cities).map((item, index) => {
            return (
              <Row key={index} id={item}>
                <Col span={2}>{item}:</Col>
                <Col span={22} className='cc-choose-city-name'>
                  {
                    (cities as any)[item].map((c: CityItem) => {
                      return (
                        <div onClick={() => clickItem(c)}
                         className='cc-choose-city-name-item' key={c.id}>{c.name}</div>
                      )
                    })
                  }
                </Col>
    ...
    >
      ...
      >
        ...
        <div style={{ marginLeft: 6 }}>
          <DownOutlined className={`cc-choose-city-icon ${flag ? 'cc-choose-city-rotate' : ''}`} />
        </div>
      </div>
    </Popover>
  )
